...
table#tbl-comment tr.level2 sub.sub-comment-writer {
	color: #8e8eff;
	font-size: 14px
}

table#tbl-comment tr.level2 sub.sub-comment-date {
	color: #ff9c8a;
	font-size: 10px
}
/*답글관련*/
...
body {
	...
	line-height: 184.61538%;
	overflow-x: hidden;
	position: relative font-family: 'RIDIBatang';
}
			...
			<div class="shadow">
				<div class="_cont detail-top">
					<div class="cols">
						...
						<div class="right-col">
							<h1 itemprop="name" style="font-weight: bold"><%=b.getBoardWriter()%>🥕
								님의
								<%=b.getBoardTitle()%></h1>
							...
							<div itemprop="offers" itemscope
								...
								<div id="comment-container">
									...
									<table id="tbl-comment">
										...
										<tr class="level1">
											<td><sub class="comment-writer"><%=c.getCommentWriter() %></sub>
												<sub class="comment-date"><%=c.getCommentDate() %></sub> <br>
												<%=c.getCommentContent() %></td>
											<td>
												<%if(loginMember!=null){%>
										...
										<%}else{%>
										<tr class="level2">
											<td><sub class="sub-comment-writer"><%=c.getCommentWriter() %></sub>
												<sub class="sub-comment-date"><%=c.getCommentDate() %></sub>
												<br> <%=c.getCommentContent() %></td>
											<td></td>
										</tr>
										<%}
											}	 
									...
									<div class="comment-editor">
										<form action="<%=request.getContextPath() %>/board/comment"
											method="post">
											<textarea style="margin-top: 2%;" name="commentContent"
												cols="55" rows="3"></textarea>
											<input type="hidden" name="boardNum"
												value="<%=b.getBoardNumber() %>"> <input
												type="hidden" name="commentWriter"
												value="<%=loginMember!=null?loginMember.getUserId():""%>">
											<input type="hidden" name="commentLevel" value="1"> <input
												type="hidden" name="refNum" value="0"> <input
												type="hidden" name="user"
												value="<%=request.getAttribute("user") %>"> <input
												type="hidden" name="love"
												value="<%=request.getAttribute("love") %>"> <input
												type="hidden" name="searchType"
												value="<%=request.getAttribute("searchType") %>"> <input
												type="hidden" name="keyword"
												value="<%=request.getAttribute("keyword") %>"> <input
												type="hidden" name="sold"
												value="<%=request.getAttribute("sold") %>"> <input
												type="hidden" name="cPage"
												value="<%=request.getAttribute("cPage") %>">
								...
								<div class="social-sharing-btn-wrapper"></div>
							</div>
						</div>
					</div>
				</div>
